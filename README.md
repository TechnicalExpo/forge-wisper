...
[ Global Shortcut: Ctrl + Space (Win) / Cmd + Space (Mac) ]
    ...
    🎙️ Speak Naturally (with pauses, corrections, or list items)
                ↓
    ⚡ Speech Recognition (Groq LPUs / Offline Whisper or Parakeet)
                ↓
...
- **🦜 Local Parakeet V3**: Optionally download the Parakeet V3 Int8 ONNX model for CPU transcription with automatic language detection. Audio remains on-device.
...
- **Local-First Processing**: Groq Cloud mode sends recorded audio to Groq for transcription. Local Whisper and Local Parakeet modes keep inference, cleanup, verification, and output on the device after model download.
